import { getCalendarEventTimes } from "@/actions/googleCalendar";
import { DAYS_OF_WEEK_IN_ORDER } from "@/constants";
import { db } from "@/drizzle/db";
import { scheduleAvailabilities } from "@/drizzle/schema";
import {
  addMinutes,
...
export async function getValidTimesFromSchedule(
  ...
  const schedule = await db.query.schedules.findFirst({
    where: ({ userId }, { eq }) => eq(userId, event.userId),
...
function getAvailabilities(
  groupedAvailabilities: Partial<
    Record<
      ...
      (typeof scheduleAvailabilities.$inferSelect)[]
    >
  >,
  date: Date,
  timezone: string
